...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([getSummary(), getExpiringSubscriptions(7)])
      .then(([s, e]) => { setSummary(s.data); setExpiring(e.data) })
      .catch(() => setError('No se pudo conectar con el servidor. Asegúrate de que el backend está corriendo.'))
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Ingresos Totales" value={fmt(summary.total_ingresos_mxn)}
          sub="Todos los tiempos" icon={<DollarSign size={20} />} color="#22c55e" trend="up" />
        <StatCard title={isProfit ? 'Ganancia Neta' : 'Pérdida Neta'} value={fmt(Math.abs(ganancia))}
          sub={isProfit ? 'Después de gastos' : 'Estás en pérdida'}
          icon={isProfit ? <TrendingUp size={20} /> : <TrendingDown size={20} />}
          color={isProfit ? '#6366f1' : '#ef4444'} trend={isProfit ? 'up' : 'down'} />
        <StatCard title="Clientes IPTV" value={String(summary.clientes_activos_iptv || 0)}
          sub="Activos actualmente" icon={<Users size={20} />} color="#06b6d4" />
        <StatCard title="Total Gastos" value={fmt(summary.total_gastos_mxn)}
          sub="Inversión total" icon={<Package size={20} />} color="#f59e0b" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="card lg:col-span-2">
          ...
          {chartData.length > 0 ? (
            ...
            <div className="h-60 flex items-center justify-center text-slate-500 text-sm text-center">
              <div><Package size={32} className="mx-auto mb-2 opacity-30" /><p>Sin datos aún. Agrega ventas para ver la gráfica.</p></div>
            </div>
          )}
        </div>
        ...
        <div className="card">
          ...
          {pieData.length > 0 ? (
            ...
            <div className="h-48 flex items-center justify-center text-slate-500 text-sm text-center">
              <div><Tv size={32} className="mx-auto mb-2 opacity-30" /><p>Sin ingresos registrados</p></div>
            </div>
          )}
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: '1px solid #2d3f58' }}>
                  {['Cliente', 'Conexiones', 'Vence', 'Días'].map(h => (
