    ...
    <div className="flex flex-col items-center">
      <svg width="200" height="115" viewBox="0 0 200 115">
        ...
        <path
          ...
          fill="none" stroke="var(--surface-2)" strokeWidth="14" strokeLinecap="round"
        />
        {/* Fill (color) */}
        <path
        ...
        <text x={cx} y={cy + 12} textAnchor="middle" fill="var(--text-muted)" fontSize="11">{label}</text>
      </svg>
      <p className="text-xs text-dim -mt-2">Salud financiera mensual</p>
    </div>
  )
}
...
export default function Presupuesto() {
  ...
  const barData = [
    { name: 'Fijos',          value: gastosFijos,      fill: 'var(--accent)' },
    { name: 'Necesarios',     value: gastosNecesarios, fill: 'var(--green)' },
    { name: 'Discrecionales', value: gastosDisc,       fill: 'var(--yellow)' },
    { name: 'Ahorro Neto',    value: Math.max(0, ahorroNeto), fill: 'var(--cyan)' },
  ]
  ...
  return (
    <div className="space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-strong">Presupuesto Personal</h1>
      ...
      {configs.length === 0 && (
        <div className="card text-center py-10">
          ...
          <p className="text-strong font-medium mb-1">No hay hoja de presupuesto conectada</p>
          <p className="text-muted text-sm mb-4">Ve a la sección <strong>Sheets Sync</strong> para conectar tu Google Sheet de presupuesto.</p>
      ...
      {showAddGasto && (
        <div className="card" style={{ borderColor: 'rgba(99,102,241,0.3)', background: 'var(--accent-soft)' }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { title: 'Ingreso Mensual',   value: fmt(ingreso),     color: 'var(--green)', icon: <DollarSign size={18}/>, sub: 'Base de cálculo' },
          { title: 'Total Gastos',       value: fmt(totalGastos), color: 'var(--yellow)', icon: <TrendingDown size={18}/>, sub: `${Math.round((totalGastos/ingreso)*100)}% del ingreso` },
          { title: 'Ahorro Neto',        value: fmt(Math.abs(ahorroNeto)), color: ahorroNeto >= 0 ? 'var(--accent)' : 'var(--red)', icon: <PiggyBank size={18}/>, sub: ahorroNeto >= 0 ? 'Disponible este mes' : '¡Gasto mayor al ingreso!' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="card">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              ...
              <div className="space-y-4">
                {[
                  { label: 'Gastos Fijos',        value: gastosFijos,      pct: pctFijo, color: 'var(--accent)' },
                  { label: 'Gastos Necesarios',    value: gastosNecesarios, pct: pctNec,  color: 'var(--green)' },
                  { label: 'Gastos Discrecionales',value: gastosDisc,       pct: pctDisc, color: 'var(--yellow)' },
                ].map(bar => (
                  <div key={bar.label}>
                    <div className="flex justify-between text-xs mb-1.5">
                      <span className="text-body font-medium">{bar.label}</span>
                      <span className="font-semibold" style={{ color: bar.color }}>
                        {fmt(bar.value)} <span className="text-dim">({bar.pct}%)</span>
                      </span>
                    </div>
          ...
          <div className="card">
            <h3 className="text-sm font-semibold text-strong mb-4">Distribución del Ingreso</h3>
        ...
        <div className="space-y-4">
          ...
          {/* Ahorro neto */}
          <div className="card" style={{ borderTop: `2px solid ${ahorroNeto >= 0 ? 'var(--accent)' : 'var(--red)'}` }}>
            <div className="flex items-center gap-2 mb-2">
              {ahorroNeto >= 0
                ? <TrendingUp size={16} className="text-indigo-400" />
                : <TrendingDown size={16} className="text-red-400" />}
              <h3 className="text-sm font-semibold text-strong">Ahorro Neto</h3>
            </div>
            <p className="text-2xl font-bold" style={{ color: ahorroNeto >= 0 ? 'var(--accent-hi)' : 'var(--red)' }}>
          ...
          {proximosGastos.length > 0 && (
            <div className="card">
              <h3 className="text-sm font-semibold text-strong mb-3">Próximos Gastos</h3>
                ...
                {proximosGastos.map((g, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between py-1 border-b last:border-0" style={{ borderColor: 'var(--border)' }}>
                      <div>
                        <p className="text-xs text-strong font-medium">{g.concepto}</p>
                        {dias !== null && (
                      ...
                      <span className="text-xs font-bold text-strong">{fmt(g.monto)}</span>
                    </div>
                  )
                })}
              </div>
          ...
          {/* Alertas */}
          {pctDisc > 70 && (
            <div className="card" style={{ borderLeft: '3px solid var(--yellow)', background: 'var(--yellow-soft)' }}>
          ...
          {ahorroNeto < 0 && (
            <div className="card" style={{ borderLeft: '3px solid var(--red)', background: 'var(--red-soft)' }}>
              <div className="flex items-start gap-2">
      ...
      <div className="card">
        <button
          ...
          className="w-full flex items-center justify-between text-sm font-semibold text-strong"
        >
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b" style={{ borderColor: 'var(--border)' }}>
                  <th className="text-left py-2 text-muted font-medium">Concepto</th>
                  <th className="text-left py-2 text-muted font-medium">Categoría</th>
                  <th className="text-left py-2 text-muted font-medium">Fecha</th>
                  <th className="text-right py-2 text-muted font-medium">Monto</th>
                </tr>
              </thead>
              <tbody>
                {gastos.map((g, i) => (
                  <tr key={i} className="table-row-hover border-b last:border-0" style={{ borderColor: 'var(--border)' }}>
                    <td className="py-2 text-strong">{g.concepto}</td>
                    <td className="py-2">
                      <span className={`badge ${g.categoria === 'fijo' ? 'badge-indigo' : g.categoria === 'necesario' ? 'badge-green' : 'badge-yellow'}`}>
                        {g.categoria}
                      </span>
                    </td>
                    <td className="py-2 text-muted">{g.fecha || '—'}</td>
                    <td className="py-2 text-right font-semibold text-strong">{fmt(g.monto)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t" style={{ borderColor: 'var(--border-hi)' }}>
                  <td colSpan={3} className="py-2 text-muted font-medium">Total</td>
                  <td className="py-2 text-right font-bold text-strong">{fmt(totalGastos)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
